...
const slackBotToken = 'xoxb-1345920338561-1342523238182-XtIOS9OXJ0oYEWnSNUb9JV4D';
...
class ActiveCode extends _common_js_runestonebase_js__WEBPACK_IMPORTED_MODULE_0__["default"] {
    ...
    /**
     * generate the elements and functions on the page
     */
    createEditor(index) {
    ...
    // generate common code codemirror
    ...
    // generate highlight code codemirror
    ...
    //
    plumber.config({
        websocketURL: "wss://plmbr.herokuapp.com",
        apiKey: "ANF6J"
    });
    // plumber.subscribe(console.log);
    // plumber.shout("Hello");

    var pipe = plumber.getPipe()
    ...
    // connect to the plumber sharedb
    this.docMyCode = pipe.getDoc(this.divid + eBookConfig.username);
    if (this.docMyCode.type == null) {
        this.docMyCode = pipe.createDoc(this.divid + eBookConfig.usernameentID, {});
    }
    const currentCodeDoc = this.docMyCode;
    const problem_id = this.divid;

    // get doc that store the latest code
    ...
    var codeSelectStart = null;
    var codeSelectEnd = null;
    ...
    $(helpButton).click(function () {
        if (raise_quest == '') {
        raise_quest = $("div[data-childcomponent='" + problem_id + "'] p:first-child").text();
        if (raise_quest == '') { raise_quest = "Not Assessment Chapter"; }
        }


        var chapter = document.title.split("—")[0];
        var editorCode = editor.getValue();
        console.log(raise_quest);
        var questionTextHtml = "<p style=\"text-align: left !important\"><strong>Book problem: </strong>" + raise_quest + "</p>" +
        "<p style=\"text-align: left !important\"><strong>Status: </strong>Unresolved</p>" +
        "<p style=\"text-align: left !important\"><strong>Chapter: </strong>" + chapter + "</p>" +
        "<p style=\"text-align: left !important\"><strong>Code: </strong></p>" +
        "<div style=\" border-radius: 10px; text-align: left !important; max-height: 100px !important; overflow: scroll !important; border: 1px solid #e0e0eb;\">";
        var codeList = editorCode.split("\n");
        codeList.forEach(code => {
        questionTextHtml += "<p style=\"font-family: 'Lucida Console', Courier, monospace;margin-left: 6px; text-align: left !important\">" + code + "</p>";
        });
        questionTextHtml += "</div><p style=\"text-align: left !important\"><strong>Discussion problem: </strong></p>";
        $("#modalBodyExample" + problem_id).html(questionTextHtml);
        $("#questModal" + problem_id).modal("show");
    });
    ...
    // modal body example
    const modalBodyExample = document.createElement("div");
    modalBodyExample.id = "modalBodyExample" + problem_id;
    $(modalBodyExample).css("background-color", "#FFFAF0");
    modalBodyDiv.appendChild(modalBodyExample);
    // modal body textarea
    ...
    // answerLinkButton.title = "Link the response and code."
    ...
    // create input(for answer) codemirror
    ...
    */
    ...
    /**
     * listening code editor copy event(for code answer link function)
     */
    editor.on('copy', function (cm, event) {
        codeSelectStart = editor.getCursor(true);
        codeSelectEnd = editor.getCursor(false);
        console.log(codeSelectStart);
        console.log(codeSelectEnd);
    });

    /**
     * listening input editor paste event(for code answer link function)
     */
    inputCodeMirror.on('paste', function (cm, event) {
        // get the cursor in input codemirror
        ...
        // check all cursor should not be null
        // start cursor and end cursor in input codemirror should not be the same, otherwise it is a common copy paste operation
        if (answerInputSelectStart == null || answerInputSelectEnd == null ||
        codeSelectStart == null || codeSelectEnd == null ||
        codeSelectStart == codeSelectEnd ||
        (answerInputSelectEnd.line == answerInputSelectStart.line && answerInputSelectEnd.ch == answerInputSelectStart.ch)) {
        codeSelectStart = null;
        codeSelectEnd = null;
        }
        else {
        // prevent common paste
        event.preventDefault();
        // get the selection text
        var selectStr = inputCodeMirror.getSelection();

        // use unique tag to replace the text
        var snippet = "[[tag]]";
        inputCodeMirror.doc.replaceRange(snippet,
            { line: answerInputSelectStart.line, ch: answerInputSelectStart.ch },
            { line: answerInputSelectEnd.line, ch: answerInputSelectEnd.ch });

        // generate link html code
        var htmlLink = document.createElement("a");
        htmlLink.innerText = selectStr;
        htmlLink.href = "";
        // use link html code to replace the unique text
        inputCodeMirror.doc.markText({ line: answerInputSelectStart.line, ch: answerInputSelectStart.ch },
            { line: answerInputSelectStart.line, ch: answerInputSelectStart.ch + snippet.length },
            { replacedWith: htmlLink });
        ...
        var newData = {
            ...
            snippet: snippet,
            originText: selectStr
        };
        codePointer.push(newData);
        answerInputSelectStart = null;
        answerInputSelectEnd = null;
        codeSelectStart = null;
        codeSelectEnd = null;
    ...
    function showAllDiscussionStat() {
        ...
        for (var i = 0; i < discussionStat.data.length; ++i) {
            if (discussionStat.data[i] == 1) { ++a; }
            if (discussionStat.data[i] == 2) { ++b; }
            if (discussionStat.data[i] == 3) { ++c; }
            if (discussionStat.data[i] == 4) { ++d; }
        }
        ...
        $(block3).css("padding", "3px");
        block3.innerHTML = "<div><Strong>Long-time unresolved posts</Strong></div>";
        for (let i = 0; i <= len / 2; ++i) {
            let b2 = document.createElement("div");
            b2.title = "Click to open the post.";
            $(b2).css("border-top", "3px solid #dbdbdb");
            $(b2).css("cursor", "pointer");
            $(b2).css("padding", "3px");
            b2.innerHTML = "Problem: " + postQuestion[i] + "<br>" + "Time: " + postTime[i];
            $(b2).mousemove(function () {
            $(b2).css("background-color", "#FFE4C4");
            });
    ...
    function changeResolve(questIndex, ListIndex, userID) {
        ...
        if (currentDocForDiscussion.data[questIndex].resolved) {
            ...
            // sync to slack, update question state in slack
            const slackBody = {
            blocks: JSON.stringify(block),
            channel: slackBotChannel,
            ...
            var maxlike = 0;
            var maxAnswer = "";
            currentDocForDiscussion.data[questIndex].chat.forEach((ans) => {
            if (ans.likes.length >= maxlike) {
                maxlike = ans.likes.length;
                maxAnswer = ans.answer;
            }
            });
            var sumBlock = [
            {
            ...
            },
            {
                ...
                {
                    "type": "mrkdwn",
                    "text": "*Summarize*: not resolved"
                }
                ]
            }
            ];
            // update sum slack in slack
            const sumSlackBody = {
            blocks: JSON.stringify(sumBlock),
            channel: slackBotChannel,
            ts: currentDocForDiscussion.data[questIndex].sumTS,
            token: slackBotToken
            };
            $.ajax({
            type: 'POST',
    ...
    */
    ...
    function showTitles() {
        ...
        const currentCodeDoc = pipe.getDoc(problem_id + eBookConfig.username);
        ...
        // disconnect sharedb and question code doc, reconnect sharedb to current code doc
        helpSession.destroy();
        currentCodeDoc.fetch(function (err) {
        if (err) throw err;
        if (currentCodeDoc.type === null) {
    ...
    function clickLikeButton(questId, answerId) {
        ...
        // if the use liked the answer, then dislike it. otherwise, the use like it
        if (userIndex > -1) {
        ...
        currentDocForDiscussion.submitOp([{
        p: [questId],
    ...
    function showQuestDetail(questId, questCode, questIndex, problem_id) {
        ...
        //document.getElementById("answerLinkButton" + problem_id).style.display = '';
        ...
        // disconnect sharedb and current code doc, connect sharedb to the question code doc
    ...
    function addNewDiscussionSession() {
        ...
        currentDocForDiscussion.fetch(function (err) {
            ...
            success: function (msg) {
                // wait for slack success and save the timestamp of the new question
                ...
                // add  a reply to the new question, it contains the code and will sync after change in runestone
                ...
                success: function (replyMsg) {
                    // wait the code reply success and save it's timestamp
                    ...
                    // send question sum reply to the question
                    const sumSlackBody = {
                    blocks: JSON.stringify(sumBlock),
                    channel: slackBotChannel,
                    thread_ts: mts,
                    ...
                    success: function (replySumMsg) {
                        // wait for the sum reply success and save the timestamp for later updating
                        var replySumObj = JSON.parse(replySumMsg);
                        var sumTS = replySumObj.ts;

                        // date to save in the db doc
                        var newData = {
                        index: dataLength,
    ...
    function addNewDiscussionAnswer() {
        ...
        // first replace the link in the answer code with the origin text
        codePointer.forEach((cp) => {
            console.log(cp);
            answerValue = answerValue.replace(cp.snippet, cp.originText);
        });

        var answerCode = editor.getValue();
        var latestNewCode = answerIndex;
        // if it is the first answer, add a default answer first, with the origin code
        if (answerIndex == 0) {
            // date save in db doc
            var newData = {
            index: answerIndex,
        ...
        $.ajax({
            ...
            success: function (msg) {
            // wait for success and save the answer timestamp

            // find the most liked answer
            var maxlike = 0;
            var maxAnswer = "";
            currentDocForDiscussion.data[currentQuestIndex].chat.forEach((ans) => {
                if (ans.likes.length >= maxlike) {
                maxlike = ans.likes.length;
                maxAnswer = ans.answer;
                }
            });
            if (currentDocForDiscussion.data[currentQuestIndex].sumTS != 0) {
                var sumBlock = [
                {
                    "type": "divider"
                },
                ...
                // send the most liked answer to the slack
                const slackBody = {
                blocks: JSON.stringify(sumBlock),
                channel: slackBotChannel,
                ts: currentDocForDiscussion.data[currentQuestIndex].sumTS,
                token: slackBotToken
                };
                $.ajax({
                type: 'POST',
    ...
    /**
     * Display the code of an answer
     */
    function showPastAnswerCode(questIndex, questId, answerIndex) {
        helpSession.destroy();
        thisCodeProblem.destroy();

        var currentChat = currentDocForDiscussion.data[questIndex].chat[answerIndex];
        editor.getDoc().setValue(currentChat.code);
    }
    ...
    function addNewCodeLink() {
        ...
        answerInputSelectStart = inputSelectStart;
        answerInputSelectEnd = inputSelectEnd;
        console.log(answerInputSelectStart);
        console.log(answerInputSelectEnd);
        console.log(codeSelectStart);
        console.log(codeSelectEnd);
        if (answerInputSelectStart != null && answerInputSelectEnd != null && answerInputSelectStart != answerInputSelectEnd && codeSelectStart != codeSelectEnd) {
        ...
        // innerHTML == "Select"
        // choose the words in the response
        var inputSelectStart = inputCodeMirror.getCursor(true);
        var inputSelectEnd = inputCodeMirror.getCursor(false);
        answerInputSelectStart = inputSelectStart;
        answerInputSelectEnd = inputSelectEnd;
